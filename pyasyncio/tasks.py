...
# 辅助生成task名称
# 这里使用itertools.count()而不是 a += 1 是因为后者并非线程安全
    ...
    def __eager_start(self):
        ...
        try:
            ...
            try:
                ...
                # 若没有执行完毕。注册任务到 调度队列 _scheduled_tasks
                else:
                    _register_task(self)

    def __step(self, exc=None):
        """
        当前task切换，然后实际执行 __step_run_and_handle_result
        :param exc:
        :return:
        """
        if self.done():
...
async def wait(fs, *, timeout=None, return_when=ALL_COMPLETED):
    ...
    Usage:
        ...
        done, pending = await pyasyncio.wait(fs)
...
async def wait_for(fut, timeout):
    ...
    #
    #     try:
    #         await pyasyncio.wait_for(func(), 0)
    #     except pyasyncio.TimeoutError:
    #         assert not func_started
    #     else:
    #         assert False
    #
    # pyasyncio.run(test_waitfor())
...
def __sleep0():
    ...
    This is a private helper for 'pyasyncio.sleep()', used
...
def ensure_future(coro_or_future, *, loop=None):
    ...
    if not coroutines.iscoroutine(coro_or_future):
        if inspect.isawaitable(coro_or_future):
            ...
            raise TypeError('An pyasyncio.Future, a coroutine or an awaitable '
                            'is required')
...
def shield(arg):
    ...
    The statement
        ...
        task = pyasyncio.create_task(something())
    ...
    """Create a function suitable for use as a task factory on an event-loop.
        ...
        Example usage:
            ...
            loop.set_task_factory(
                pyasyncio.create_eager_task_factory(my_task_constructor))
        ...
        used. E.g. `loop.set_task_factory(pyasyncio.eager_task_factory)`.
        """
...
# 这里表示急需调度的任务
# 这两组任务集共同包含对完整活动任务集的引用。预先执行的任务使用更快的 regular set 作为优化，但如果任务在 IO 上阻塞，则可能会升级到 WeakSet。
# 需要调度的任务
_scheduled_tasks = weakref.WeakSet()
_eager_tasks = set()

# 字典，包含所有线程当前所有运行中的事件循环的当前task
# 就是各个协程当前正在运行的任务
# 软件CPU
# {EventLoop: Task}
_current_tasks = {}
...
def _register_task(task):
    """Register an pyasyncio Task scheduled to run on an event loop."""
    _scheduled_tasks.add(task)


def _register_eager_task(task):
    """Register an pyasyncio Task about to be eagerly executed."""
    _eager_tasks.add(task)


def _enter_task(loop, task):
    """
    修改_current_tasks表示该任务成为当前任务并开始执行
    """
...
def _leave_task(loop, task):
    """
    修改_current_tasks表示该任务不再是当前任务并退出执行
    """
    current_task = _current_tasks.get(loop)
