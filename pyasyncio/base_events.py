...
# 在执行已取消的句柄之前，_scheduled计时器句柄的最小数量。
_MIN_SCHEDULED_TIMER_HANDLES = 100

# 在执行已取消句柄的清理之前_scheduled取消的计时器句柄的最小比例。
...
# 检查当前socket库是否有TCP_NODELAY属性
# TCP_NODELAY 是 TCP 套接字 的一个选项
# 用于关闭 Nagle 算法（也称为延迟确认算法）。Nagle 算法是一种用于减少网络上小数据包数量的算法，它通过合并小的数据包来减少网络拥塞。但是，
# 在某些情况下，比如需要实时传输数据的应用中，Nagle 算法可能会导致不必要的延迟，因为数据包会等待更多的数据到达后才发送。
if hasattr(socket, 'TCP_NODELAY'):
    def _set_nodelay(sock):
        if (sock.family in {socket.AF_INET, socket.AF_INET6} and  # 检查套接字是否为 IPv4 或 IPv6协议族
                sock.type == socket.SOCK_STREAM and  # 检查套接字是否为流式套接字，即是否为TCP
                sock.proto == socket.IPPROTO_TCP): # 检查套接字是否使用TCP协议
            # 上述条件满足，说明是一个流式套接字，可以设置 TCP_NODELAY 选项
            # 使用 sock.setsockopt 方法设置 TCP_NODELAY 选项为 1，即关闭 Nagle 算法。
            sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
else:
    # 如果 socket 模块没有 TCP_NODELAY 属性，定义的 _set_nodelay 函数什么也不做（pass），因为当前环境不支持这个选项。
    def _set_nodelay(sock):
        pass
...
class BaseEventLoop(events.AbstractEventLoop):
    ...
    def call_soon(self, callback, *args, context=None):
        """
        尽快安排回调
        回调函数注册：基于回调函数创建handle对象，handle对象被放置在一个 FIFO 队列，按照入队顺序执行，每个回调对应的仅执行一次
        位置参数将会在回调函数被调用时传递给回调函数
        """
        self._check_closed()
        if self._debug:
            self._check_thread()
    ...
    async def sendfile(self, transport, file, offset=0, count=None,
        ...
        fallback set to True makes pyasyncio to manually read and send
    ...
    def _run_once(self):
        ...
        # 调度句柄总数量大于最小阈值，并且被取消句柄数量占总数量比例大于最小阈值，就重建 self._scheduled 堆
        ...
        # asyncio中只有这一个地方会对callback进行真正地调用，其他地方都是在添加到ready中
        # 注意：此处将运行所有调度的callback，但不会运行这次运行的回调的回调
        # 回调的回调将在下次运行（在下一次I/O poll之后）
        # Use an idiom that is thread-safe without using locks. --> 使用无锁编程，例如CAS等

        # 获取ready队列长度
        n_todo = len(self._ready)
        # 对前 n_todo 个准备就绪的handle进行处理
